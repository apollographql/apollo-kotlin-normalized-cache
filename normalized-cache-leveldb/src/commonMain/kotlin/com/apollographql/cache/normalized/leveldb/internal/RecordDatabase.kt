package com.apollographql.cache.normalized.leveldb.internal
...
internal class RecordDatabase(
  ...
  companion object {
    ...
    suspend fun bind(name: String) {
      mutex.withLock {
        check(!boundNames.contains(name)) { "The file $name is already bound to another LevelDBNormalizedCache. Call LevelDBNormalizedCache.close() to release it." }
        boundNames.add(name)
      }
    }
