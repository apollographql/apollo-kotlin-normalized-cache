package com.apollographql.cache.normalized.leveldb
...
import com.apollographql.cache.normalized.leveldb.internal.RecordDatabase
import com.apollographql.cache.normalized.leveldb.internal.RecordSerializer
import kotlinx.coroutines.flow.Flow
import kotlinx.coroutines.flow.emptyFlow
import kotlinx.coroutines.flow.toList
import kotlin.reflect.KClass

class LevelDBNormalizedCache internal constructor(
  ...
  private suspend fun internalDeleteRecords(keys: Collection<String>, cascade: Boolean): Int {
    ...
    recordDatabase.deleteRecords(keys + referencedKeys)
    return keys.size + referencedKeys.size
  }
  ...
  private suspend fun internalUpdateRecords(
    ...
    return if (cacheHeaders.headerValue(ApolloCacheHeaders.SKIP_MERGE) == "true") {
      ...
      recordDatabase.insertOrUpdateRecords(records)
      emptySet()
    } else {
      ...
      val recordToInsertOrUpdate = mutableListOf<Record>()
      val changedKeys = records.flatMap { record ->
        val existingRecord = existingRecords[record.key]
        if (existingRecord == null) {
          recordToInsertOrUpdate.add(record.withDates(receivedDate = receivedDate, expirationDate = expirationDate))
          record.fieldKeys()
        } else {
          ...
          if (mergedRecord.isNotEmpty()) {
            recordToInsertOrUpdate.add(mergedRecord.withDates(receivedDate = receivedDate, expirationDate = expirationDate))
          }
          changedKeys
        }
      }.toSet()
      recordDatabase.insertOrUpdateRecords(recordToInsertOrUpdate)
      changedKeys
    }
  }
  ...
  private suspend fun selectRecords(keys: Collection<CacheKey>): List<Record> {
    recordDatabase.init()
    return recordDatabase.selectRecords(keys.map { it.key })
  }
