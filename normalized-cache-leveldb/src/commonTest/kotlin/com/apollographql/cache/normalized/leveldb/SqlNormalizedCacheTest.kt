...
class SqlNormalizedCacheTest {
  private val cache: NormalizedCache = LevelDBNormalizedCacheFactory().create()
...
//    val badCache = LevelDBNormalizedCache(RecordDatabase(BadDriver, null))
...
//  }
  ...
  fun testSizeOfRecord() = runTest {
    ...
    val normalizedCache = LevelDBNormalizedCacheFactory().create().apply { clearAll() }
  ...
  fun cannotReuseNameWithoutClose() = runTest {
    ...
    val cache1 = LevelDBNormalizedCacheFactory().create()
    cache1.clearAll()

    val cache2 = LevelDBNormalizedCacheFactory().create()
    cache2.clearAll()
  ...
  fun canUseDifferentNames() = runTest {
    ...
    var exception: Throwable? = null
    apolloExceptionHandler = { exception = it }
    val cache1 = LevelDBNormalizedCacheFactory("a.db").create()
    cache1.clearAll()

    val cache2 = LevelDBNormalizedCacheFactory("b.db").create()
    cache2.clearAll()
