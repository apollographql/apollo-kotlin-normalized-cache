...
class DiskNormalizedCacheTest {
  private val cache: NormalizedCache = DiskNormalizedCacheFactory().create()
...
//    val badCache = DiskNormalizedCache(RecordDatabase(BadDriver, null))
...
//  }
  ...
  fun testSizeOfRecord() = runTest {
    ...
    val normalizedCache = DiskNormalizedCacheFactory().create().apply { clearAll() }
  ...
  fun cannotReuseNameWithoutClose() = runTest {
    ...
    val cache1 = DiskNormalizedCacheFactory().create()
    cache1.clearAll()

    val cache2 = DiskNormalizedCacheFactory().create()
    cache2.clearAll()
  ...
  fun canUseDifferentNames() = runTest {
    ...
    var exception: Throwable? = null
    apolloExceptionHandler = { exception = it }
    val cache1 = DiskNormalizedCacheFactory("a.db").create()
    cache1.clearAll()

    val cache2 = DiskNormalizedCacheFactory("b.db").create()
    cache2.clearAll()
