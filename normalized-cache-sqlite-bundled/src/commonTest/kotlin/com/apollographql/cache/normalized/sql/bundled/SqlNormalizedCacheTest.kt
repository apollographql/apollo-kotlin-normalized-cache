...
class SqlNormalizedCacheTest {
  ...
  fun exceptionCallsExceptionHandler() = runTest(before = { setUp() }, after = { tearDown() }) {
    val badCache = SqlNormalizedCache(RecordDatabase("/nonexistent/path/db"))
    ...
    assertEquals("Error code: 14", throwable.cause!!.message)
  ...
  fun cannotReuseNameWithoutClose() = runTest {
    ...
    cache2.clearAll()
    assertEquals("The file /Users/bod/.apollo/apollo.db is already bound to another SqlNormalizedCache. Call SqlNormalizedCache.close() to release it.", exception?.cause?.message)
