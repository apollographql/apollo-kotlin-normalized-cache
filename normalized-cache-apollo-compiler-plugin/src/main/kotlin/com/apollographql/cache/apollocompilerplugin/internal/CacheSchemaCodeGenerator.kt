...
internal class CacheSchemaCodeGenerator(
  ...
  override fun generate(schema: GQLDocument, outputDirectory: File) {
    ...
    val issueSeverities = environment.arguments["com.apollographql.cache.issueSeverities"]
    if (issueSeverities != null) {
      if (issueSeverities !is Map<*, *>) {
        throw IllegalArgumentException("com.apollographql.cache.issueSeverities argument must be a Map<String, String>")
      }
    }
    ...
    if (edgeNodeTypeWithoutKeyFieldsSeverity != "ignore") {
      val connectionTypeIssues = validSchema.validateConnectionTypes(connectionTypes, typePolicies)
      for (issue in connectionTypeIssues) {
        issueLogger.logIssue(issue, null)
      }
      if (issueLogger.hasIssues && edgeNodeTypeWithoutKeyFieldsSeverity == "error") {
        throw IllegalStateException("Issues found while validating connection types (see logs above)")
      }
    }
  ...
  private fun cacheFunction(): FunSpec {
    return FunSpec.builder("cache")
        ...
        .addParameter(
            ParameterSpec.builder("keyScope", Symbols.CacheKeyScope)
                .defaultValue("CacheKey.Scope.TYPE").build(),
        )
        .addParameter(
            ParameterSpec.builder("defaultMaxAge", Symbols.KotlinDuration)
                .defaultValue("%T.INFINITE", Symbols.KotlinDuration)
                .build(),
        )
        .addParameter(
            ParameterSpec.builder("enableOptimisticUpdates", BOOLEAN)
                .defaultValue("false")
                .build(),
        )
        .addParameter(
            ParameterSpec.builder("writeToCacheAsynchronously", BOOLEAN)
                .defaultValue("false")
                .build(),
        )
